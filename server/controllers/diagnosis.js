...
export default function diagnosisController({ diagnosisService, csvEscape = (v)=>String(v||'') } = {}) {
  const router = express.Router()

  router.get('/generate', async (req, res) => {
    try {
      if (!diagnosisService || typeof diagnosisService.generatePrompts !== 'function') return res.status(500).json({ ok: false, error: 'Ollama not configured' })
  ...
  router.post('/validate-bulo', async (req, res) => {
    try {
      const out = await diagnosisService.validateBulo(req.body && req.body.answer)
  ...
  router.get('/results', async (req, res) => {
    try { const docs = await diagnosisService.listResults(req.query.classId); return res.json(docs) } catch (e) { return res.status(500).json({ ok: false, error: String(e) }) }
  })
