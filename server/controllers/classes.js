...
export default function classesController({ classesRepo, classService } = {}) {
  ...
  const service =
    ...
    (classesRepo
      ? {
          list: (q) => classesRepo.find(q),
          get: (id) => classesRepo.findById(id),
          create: async (p) => {
            if (!p.id)
              p.id = Math.random().toString(36).substring(2, 8).toUpperCase();
            p.created_at = p.created_at || new Date();
            await classesRepo.upsert(p);
            return { ok: true, id: p.id };
          },
          update: (id, u) => classesRepo.update(id, u),
          delete: (id) => classesRepo.deleteById(id),
        }
      : null);
