...
export default function questionsController({ questionService } = {}) {
  ...
  router.post('/:id/reveal', async (req, res) => {
    ...
    const { classId, correctAnswer, points = 100, activeQuestion } = req.body || {}
    ...
    if ((evalMode === 'mcq' || evalMode === 'redflags') && typeof correctAnswer === 'undefined') {
      return res.status(400).json({ error: 'classId and correctAnswer required for this question type' })
    }
    try {
