...
export default class AnswerService {
  ...
  async submitAnswer({
    ...
    try {
      ...
      try {
        if (typeof this.broadcast === "function") this.broadcast(agg, classId);
      } catch (e) {
        /* ignore */
      }
    } catch (e) {
    ...
    try {
      ...
      const computeAndApplyAward = async (
        rawScore,
        feedback = "",
        source = "server",
      ) => {
        const raw = Number(rawScore || 0);
        const scoreFraction = Math.max(
          0,
        ...
        const totalDurationSec =
          questionPayload && Number(questionPayload.duration)
            ...
            : questionPayload && Number(questionPayload.points)
              ? 30
              : 30;
        const startedAt =
        ...
        if (
          ...
          typeof this.participantsRepo.incScore === "function"
        ) {
          try {
      ...
      if (evaluation && (evalMode === "open" || evalMode === "prompt")) {
        ...
        this.evaluator &&
        typeof this.evaluator.evaluate === "function"
      ) {
