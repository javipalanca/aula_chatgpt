...
export default class WSManager {
  ...
  _onConnection(ws) {
    if (
      ...
      typeof this.broadcastService.registerClient === "function"
    )
      this.broadcastService.registerClient(ws);
  ...
  async _onMessage(ws, msg) {
    ...
    try {
      if (obj.type === "subscribe" && obj.classId)
        return await this._handleSubscribe(ws, obj);
      if (obj.type === "ping" && obj.classId && obj.sessionId)
        return await this._handlePing(ws, obj);
      if (
        obj.type === "answer" &&
        obj.classId &&
        obj.sessionId &&
        ...
        return await this._handleAnswer(ws, obj);
      if (obj.type === "unsubscribe" && obj.classId)
        return await this._handleUnsubscribe(ws, obj);
      if (
        obj.type === "reveal" &&
        obj.classId &&
        obj.questionId &&
        ...
        return await this._handleReveal(ws, obj);
    } catch (e) {
      try {
  ...
  async _handleSubscribe(ws, obj) {
    ...
    if (
      this.broadcastService &&
      typeof this.broadcastService.subscribe === "function"
    )
      this.broadcastService.subscribe(ws, cid);
    try {
    ...
    if (
      ...
      typeof this.participantsService.handleSubscribe === "function"
    ) {
      await this.participantsService.handleSubscribe({
        classId: cid,
        sessionId,
        role,
    ...
    try {
      ws.send(JSON.stringify({ type: "subscribed", classId: cid, role }));
    } catch (e) {
      /* ignore */
    }
    ...
    try {
      ...
      if (role === "student" && active && active.question) {
        ...
        const totalDuration =
          q.duration && Number(q.duration) ? Number(q.duration) : 30;
        const elapsed = Math.floor(
          (Date.now() - (active.startedAt || Date.now())) / 1000,
        );
      ...
      try {
        if (
          role === "teacher" &&
          this.participantsService &&
          typeof this.participantsService.fetchConnectedParticipants ===
            "function"
        ) {
          const docs =
            await this.participantsService.fetchConnectedParticipants(cid);
          try {
            ws.send(
              JSON.stringify({
  ...
  async _handlePing(ws, obj) {
    ...
    if (
      this.participantsService &&
      typeof this.participantsService.handlePing === "function"
    ) {
  ...
  async _handleAnswer(ws, obj) {
    ...
    if (
      this.answerService &&
      typeof this.answerService.submitAnswer === "function"
    ) {
      await this.answerService.submitAnswer({
        ...
        activeQuestion: active
          ? { question: active.question, startedAt: active.startedAt }
          : null,
      });
    }
  }
  ...
  async _handleUnsubscribe(ws, obj) {
    ...
    if (
      this.broadcastService &&
      typeof this.broadcastService.unsubscribe === "function"
    )
      this.broadcastService.unsubscribe(ws, cid);
    if (
      sessionId &&
      this.participantsService &&
      typeof this.participantsService.handleDisconnect === "function"
    ) {
  ...
  _onClose(ws) {
    ...
    try {
      const set =
        this.broadcastService &&
        this.broadcastService.wsToClasses &&
        this.broadcastService.wsToClasses.get &&
        this.broadcastService.wsToClasses.get(ws);
      if (
        set &&
        ws &&
        ws._sessionId &&
        ...
        for (const cid of set) {
          try {
            this.participantsService.handleDisconnect(cid, ws._sessionId);
          } catch (e) {
            /* ignore per-socket error */
          }
        }
      }
    } catch (e) {
    ...
    if (
      this.broadcastService &&
      typeof this.broadcastService.unregisterClient === "function"
    ) {
      try {
        this.broadcastService.unregisterClient(ws);
      } catch (e) {
        /* ignore */
      }
    }
