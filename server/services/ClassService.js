...
export default class ClassService {
  ...
  async create(payload = {}) {
    if (!payload.id)
      payload.id = Math.random().toString(36).substring(2, 8).toUpperCase();
  ...
  async count(q = {}) {
    if (typeof this.classesRepo.count === "function")
      return this.classesRepo.count(q);
    return 0;
  }
  ...
  async resetClass(id) {
    ...
    const metaToSet =
      typeof this.getDefaultMeta === "function" ? this.getDefaultMeta() : {};
    await this.update(id, { meta: metaToSet });

    // delete answers if possible
    if (
      this.answersRepo &&
      typeof this.answersRepo.deleteByClass === "function"
    ) {
      try {
        await this.answersRepo.deleteByClass(id);
      } catch (e) {
        console.warn("ClassService.resetClass: deleteByClass failed", e);
      }
    }
    ...
    if (
      ...
      typeof this.participantService.resetScores === "function"
    ) {
      try {
        await this.participantService.resetScores(id);
      } catch (e) {
        console.warn("ClassService.resetClass: resetScores failed", e);
      }
    }
    ...
    try {
      if (
        ...
        typeof this.broadcastService.publish === "function"
      ) {
        const cls = await this.get(id);
        this.broadcastService.publish(
          { type: "class-reset", classId: id, class: cls },
          id,
        );
        if (
          this.participantService &&
          typeof this.participantService.fetchConnectedParticipants ===
            "function"
        ) {
          ...
          const parts =
            await this.participantService.fetchConnectedParticipants(id, {
              includeDisconnected: false,
            });
          this.broadcastService.publish(
            { type: "participants-updated", classId: id, participants: parts },
            id,
          );
        }
      }
    } catch (e) {
      console.warn("ClassService.resetClass: broadcast failed", e);
    }
