...
export default class ParticipantService {
  constructor({
    participantsRepo,
    broadcast = null,
    participantLastPersist = null,
    participantLastBroadcast = null,
    options = {},
  } = {}) {
  ...
  async saveParticipant(payload = {}) {
    ...
    const isScoreOp =
      typeof payload.scoreDelta !== "undefined" ||
    ...
    if (typeof payload.scoreDelta !== "undefined") {
      ...
      try {
        this.participantLastPersist.set(pKey, Date.now());
      } catch (e) {
        /* ignore */
      }
    ...
    try {
      if (typeof this.broadcast === "function") {
        ...
        try {
          console.debug(
            "ParticipantService.saveParticipant: broadcasting participants-updated",
            { classId: payload.classId, count: (docs && docs.length) || 0 },
          );
        } catch (e) {
          /* ignore */
        }
  ...
  async resetScores(classId) {
    ...
    try {
      if (typeof this.broadcast === "function") {
        ...
        this.broadcast(
          { type: "participants-updated", classId, participants: docs },
          classId,
        );
      }
    } catch (e) {
    ...
    if (sessionId && role === "student") {
      ...
      try {
        if (typeof this.broadcast === "function") {
          ...
          try {
            console.debug(
              "ParticipantService.handleSubscribe: broadcasting participants-updated",
              {
                classId,
                count: (docs && docs.length) || 0,
                sessionId,
                displayName: toSet.displayName,
              },
            );
          } catch (e) {
            /* ignore */
  ...
  async handlePing(classId, sessionId) {
    ...
    const prev = await this.participantsRepo.findOneByClassSession(
      classId,
      sessionId,
    );
      ...
      await this.participantsRepo.upsert({
        ...
        displayName:
          prev && prev.displayName
            ? prev.displayName
            : `Alumno-${String(sessionId).slice(0, 5)}`,
      });
      try {
        this.participantLastPersist.set(`${classId}:${sessionId}`, Date.now());
      } catch (e) {
        /* ignore */
      }
      try {
  ...
  async handleDisconnect(classId, sessionId) {
    ...
    try {
      if (typeof this.broadcast === "function") {
        this.broadcast(
          { type: "participant-disconnected", classId, sessionId },
          classId,
        );
        const docs = await this.fetchConnectedParticipants(classId);
        this.broadcast(
    ...
    try {
      this.participantLastPersist.delete(`${classId}:${sessionId}`);
    } catch (e) {
      /* ignore */
    }
    try {
      this.participantLastBroadcast.delete(`${classId}:${sessionId}`);
    } catch (e) {
      /* ignore */
    }
  }
