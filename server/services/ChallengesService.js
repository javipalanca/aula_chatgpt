export default class ChallengesService {
  ...
  async upsert(payload = {}) {
    ...
    const looksLikeGameEnd =
      payload &&
      ((payload.payload && payload.payload.type === "game-ended") ||
        (typeof payload.title === "string" &&
          /juego terminado/i.test(payload.title)));
    if (looksLikeGameEnd) {
    ...
    if (publicQuestion.payload && typeof publicQuestion.payload === "object") {
      ...
      if (typeof publicQuestion.payload.correctAnswer !== "undefined")
        delete publicQuestion.payload.correctAnswer;
      if (typeof publicQuestion.payload.duration !== "undefined")
        publicQuestion.duration = publicQuestion.payload.duration;
    }
    try {
      if (
        this.activeQuestions &&
        typeof this.activeQuestions.set === "function"
      )
        this.activeQuestions.set(payload.classId, {
