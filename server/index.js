...
try {
  ...
  const participantService = new ParticipantService({
    participantsRepo,
    broadcast: (d, cid) => broadcastService.publish(d, cid),
    participantLastPersist,
    participantLastBroadcast,
    options: {
      minPersistMs: PARTICIPANT_MIN_PERSIST_MS,
      minBroadcastMs: PARTICIPANT_BROADCAST_MIN_MS,
    },
  });
  ...
  const diagnosisService = new DiagnosisService({
    diagnosisRepo: diagnosisResults,
    ollamaUrl: OLLAMA_URL,
    ollamaModel: OLLAMA_MODEL,
  });
  ...
  fetchConnectedParticipants = async (classId, opts = {}) =>
    participantService.fetchConnectedParticipants(classId, opts);
  ...
  const participantsController = participantsControllerFactory({
    participantService,
  ...
  const challengesController = challengesControllerFactory({
    challengesService,
  });
  ...
  const llmController = llmControllerFactory({
    evaluator: llmEvaluator,
    ollamaConfig: { url: OLLAMA_URL, model: OLLAMA_MODEL },
    fetchImpl: fetch,
  });
  ...
  app.get("/api/debug/dbstats", async (req, res) => {
    try {
      ...
      const [classesCount, participantsCount, challengesCount, diagnosisCount] =
        await Promise.all([
          typeof classesRepo.count === "function"
            ? classesRepo.count()
            : Promise.resolve(0),
  ...
  const server = app.listen(PORT, () =>
    console.log("Aula proxy server listening on", PORT),
  );
  ...
  const wsManager = new WSManager({
    ...
    answerService,
    questionService,
    fetchActiveQuestion: async (cid) => activeQuestions.get(cid),
    broadcastService,
  });
      ...
      try {
        for (const s of _connections) {
          try {
            ...
            console.debug("error destroying connection during forced exit", e);
          }
        }
      } catch (e) {
        console.debug("error destroying connections during forced exit", e);
      }
      process.exit(1);
    }, 2000);
    ...
    try {
      if (wsManager && typeof wsManager.close === "function") {
        try {
          ...
          console.error("Error closing WS manager", e);
        }
      }
      ...
      try {
        ...
        console.debug("error destroying lingering sockets", e);
      }
