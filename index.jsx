...
import { Brain, Sparkles, ShieldCheck, ThumbsUp, ThumbsDown, Lightbulb, Star, RefreshCw, Award, Link as LinkIcon, HelpCircle, Target, Pencil, CheckCircle2, XCircle, Rocket, BookOpen, Timer, Settings } from "lucide-react";
...
const VERIF_QUIZ = [
  {
    ...
    a: 2,
    explain: "Los datos pueden estar desactualizados o ser incorrectos: contrasta con fuentes fiables.",
  },
  {
    ...
    a: 0,
    explain: "Solicita fuentes concretas y compruébalas; desconfía de bibliografías inventadas.",
  },
  {
    ...
    a: 1,
    explain: "Reformular, pedir pasos o contrastar suele mejorar la calidad y detectar fallos.",
  },
  {
    ...
    a: 0,
    explain: "Evita datos sensibles (DNI, direcciones, contraseñas, salud, etc.).",
  },
  {
    ...
    a: 1,
    explain: "La transparencia es clave: explica cómo la usaste y qué parte es tuya.",
  },
];
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-indigo-50 text-slate-800">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 flex flex-col gap-6">
          ...
          {tab === "builder" && (
            <PromptBuilder onScore={(n) => { addPoints(n, "+"+n+" puntos por buen prompt"); unlockBadge("Constructor/a de Prompts"); }} />
          )}
          {tab === "etica" && (
            <EthicsGame onScore={(n) => { n>0? addPoints(n, "+"+n+" puntos en Ética") : resetStreak(); if (n>=8) unlockBadge("Guardián/a Ético/a"); }} />
          )}
          {tab === "verificacion" && (
            <VerifyQuiz onScore={(n) => { addPoints(n, "+"+n+" puntos en Verificación"); if (n>=4) unlockBadge("Detective de Fuentes"); }} />
          )}
          {tab === "mejora" && (
            <ImprovePrompt onScore={(n) => { addPoints(n, "+"+n+" puntos por mejorar"); if (n>=2) unlockBadge("Editor/a de Preguntas"); }} />
          )}
          {tab === "diagnostico" && (
            <Diagnosis onScore={(ok) => { addPoints(ok?3:0, ok?"+3 puntos por diagnosticar":undefined); if (ok) unlockBadge("Cazador/a de Fakes"); }} />
          )}
        </section>
  ...
  return (
    <button
      ...
      className={clsx(
        ...
        is ? "bg-blue-600 text-white" : "hover:bg-slate-100 text-slate-700 border border-slate-200"
      )}
      aria-pressed={is}
    >
      <Icon size={16} /> {label}
...
function PromptBuilder({ onScore }) {
  ...
  return (
    <FancyCard>
      <h2 className="text-xl font-black mb-1 flex items-center gap-2"><Sparkles className="text-blue-600"/> Constructor de Prompts</h2>
      <p className="text-sm text-slate-600 mb-4">Completa los campos para formular una petición clara y útil. ¡Lo que escribas abajo será tu <i>prompt</i> final!</p>
      <div className="grid md:grid-cols-2 gap-4">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select label="Formato" value={format} setValue={setFormat} options={["párrafos claros","lista con viñetas","esquema","tabla"]} />
            <Select label="Tono" value={tone} setValue={setTone} options={["neutral","didáctico","entusiasta","formal","ameno"]} />
          </div>
          <div className="grid grid-cols-3 gap-3 items-end">
            <Number label="Límite (palabras)" value={limit} setValue={setLimit} min={50} max={800} step={50} />
            <Select label="Idioma" value={lang} setValue={setLang} options={["español","valenciano","inglés"]} />
            <div className="flex flex-col gap-2">
              <Toggle checked={steps} setChecked={setSteps} label="Paso a paso"/>
              <Toggle checked={examples} setChecked={setExamples} label="Con ejemplos"/>
            </div>
          </div>
        </div>
        <div className="space-y-3">
          <label className="text-sm font-semibold text-slate-700">Vista previa del prompt</label>
...
function EthicsGame({ onScore }) {
  ...
  function restart() {
    setI(0); setRight(0); setFinished(false);
  }

  if (finished) {
    return (
      <FancyCard>
        <h2 className="text-xl font-black mb-2 flex items-center gap-2"><ShieldCheck className="text-green-600"/> Ética y Seguridad</h2>
        <p className="text-slate-700">¡Has acertado {right} de {ETHICS_SCENARIOS.length}! Repasa las explicaciones y vuelve a intentarlo si quieres mejorar tu marca.</p>
        <div className="mt-4 flex gap-2">
          <Button onClick={restart} variant="primary"><RefreshCw size={16} className="inline mr-2"/>Reintentar</Button>
        </div>
        <div className="mt-4 grid md:grid-cols-2 gap-4">
          {ETHICS_SCENARIOS.map((s, idx) => (
            <div key={idx} className="p-3 rounded-xl border border-slate-200">
              ...
              <p className={clsx("mt-1 text-sm", s.good ? "text-green-700" : "text-rose-700")}>{s.good ? "Adecuado" : "No adecuado"} • {s.why}</p>
            </div>
          ))}
        </div>
      </FancyCard>
  ...
  return (
    <FancyCard>
      ...
      <div className="flex items-center justify-between mb-2">
        <Pill tone="amber" icon={Timer}>Pregunta {i+1} / {ETHICS_SCENARIOS.length}</Pill>
        <Pill tone="purple" icon={Star}>Aciertos: {right}</Pill>
      </div>
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-lg font-medium">{sc.text}</div>
      <div className="mt-4 flex gap-3">
        <Button onClick={() => answer(true)} variant="success"><ThumbsUp className="inline mr-2" size={16}/> Adecuado</Button>
        <Button onClick={() => answer(false)} variant="danger"><ThumbsDown className="inline mr-2" size={16}/> No adecuado</Button>
      </div>
      <p className="mt-3 text-sm text-slate-600">Pista: piensa en honestidad académica, seguridad, legalidad, privacidad e impacto en otras personas.</p>
    </FancyCard>
  );
}
...
function VerifyQuiz({ onScore }) {
  ...
  function set(i, v) {
    const copy = [...answers];
    copy[i] = v; setAnswers(copy);
  }

  function finish() {
    let score = 0; answers.forEach((a, i) => { if (a === VERIF_QUIZ[i].a) score++; });
    setDone(true); onScore(score);
  }

  return (
    <FancyCard>
      <h2 className="text-xl font-black mb-2 flex items-center gap-2"><BookOpen className="text-indigo-600"/> Verificación de Fuentes</h2>
        ...
        {VERIF_QUIZ.map((q, i) => (
          <li key={i} className="p-3 rounded-xl border border-slate-200">
            ...
            <div className="grid gap-2">
              {q.options.map((op, j) => (
                <label key={j} className={clsx("flex items-start gap-2 p-2 rounded-xl border", answers[i]===j?"border-blue-600 bg-blue-50":"border-slate-200 hover:bg-slate-50")}> 
      ...
      <div className="mt-4 flex gap-2">
        {!done ? (
          <Button onClick={finish} variant="primary">Corregir</Button>
        ) : (
          <Button onClick={() => { setAnswers(Array(VERIF_QUIZ.length).fill(null)); setDone(false); }} variant="ghost"><RefreshCw className="inline mr-2" size={16}/>Reiniciar</Button>
        )}
      </div>
    </FancyCard>
  );
...
function ImprovePrompt({ onScore }) {
  ...
  function evaluate() {
    ...
    if (/paso a paso|pregúntame|preguntame|si falta información/i.test(t)) score++;
  ...
  return (
    <FancyCard>
      <h2 className="text-xl font-black mb-2 flex items-center gap-2"><Pencil className="text-emerald-600"/> Redacta Mejor</h2>
      <p className="text-sm text-slate-600 mb-3">Convierte una mala petición en un buen <i>prompt</i>. Usa rol, tarea, contexto, tono, formato, límites y pide pasos/preguntas si faltan datos.</p>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
          <label className="text-sm font-semibold text-slate-700">Petición pobre (ejemplo)</label>
          <div className="p-3 rounded-xl bg-rose-50 border border-rose-200 text-rose-900">{item.bad}</div>
          <p className="mt-2 text-xs text-rose-700">Pista: {item.tip}</p>
        </div>
        <div>
          <label className="text-sm font-semibold text-slate-700">Tu versión mejorada</label>
          <textarea value={draft} onChange={(e)=>setDraft(e.target.value)} className="w-full min-h-[140px] p-3 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-400" placeholder="Escribe aquí tu mejor prompt…"/>
          <div className="mt-2 flex gap-2">
            <Button variant="success" onClick={evaluate}><CheckCircle2 className="inline mr-2" size={16}/>Evaluar</Button>
            <Button variant="ghost" onClick={next}><RefreshCw className="inline mr-2" size={16}/>Otro reto</Button>
          </div>
          {checked && (
            <p className="mt-2 text-sm text-slate-700">Puntos otorgados según heurística (longitud, rol, formato, pasos, contexto). ¡Sigue practicando!</p>
          )}
        </div>
      </div>
    </FancyCard>
...
function Diagnosis({ onScore }) {
  ...
  return (
    <FancyCard>
      <h2 className="text-xl font-black mb-2 flex items-center gap-2"><HelpCircle className="text-amber-600"/> Diagnóstico de Respuestas</h2>
      <p className="text-sm text-slate-600 mb-3">Lee la respuesta y marca las señales de alerta que detectes.</p>
      <div className="grid md:grid-cols-2 gap-4">
        <div>
        ...
        <div>
          ...
          <div className="grid gap-2">
            {RED_FLAGS_SAMPLE.checks.map((c) => (
              <label key={c.id} className={clsx("flex items-start gap-2 p-2 rounded-xl border", selected.includes(c.id)?"border-blue-600 bg-blue-50":"border-slate-200 hover:bg-slate-50")}> 
          ...
          <div className="mt-2 flex gap-2">
            ...
            <Button variant="ghost" onClick={()=>{ setSelected([]); setChecked(false); }}><RefreshCw className="inline mr-2" size={16}/>Reiniciar</Button>
          </div>
          {checked && (
            <p className="mt-2 text-sm">{selected.sort().join(", ")===RED_FLAGS_SAMPLE.correct.sort().join(", ") ? (
              <span className="text-green-700 font-semibold flex items-center gap-2"><CheckCircle2/> ¡Bien visto! Esas eran las banderas correctas.</span>
            ) : (
  ...
  return (
    <footer className="border-t border-slate-200 mt-8">
      <div className="max-w-6xl mx-auto px-4 py-6 text-sm text-slate-600 flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-4">
        <span>Hecho con <span className="text-rose-600">❤</span> para aprender IA responsable.</span>
        <span className="opacity-60">Consejo: si usas esta app en clase, proyecta y trabajad por parejas.</span>
      </div>
    </footer>
  );
}
...
function toast(message, delta) {
  listeners.forEach((fn) => fn({ message, delta, id: Math.random().toString(36).slice(2) }));
}
function Toaster() {
  ...
  return (
    <div className="fixed bottom-4 right-4 flex flex-col gap-2 z-50">
      {items.map((t) => (
        <div key={t.id} className={clsx("px-4 py-2 rounded-xl shadow-lg border text-sm",
          t.delta>0?"bg-emerald-600 text-white border-emerald-700":"bg-slate-900 text-white border-slate-800")}>{t.message}</div>
      ))}
    </div>
  );
}
