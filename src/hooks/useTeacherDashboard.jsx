...
export default function useTeacherDashboard() {
   ...
   */
  const buildBlock = (id, name, items, mapper) => ({
    id,
    name,
    questions: items.map(mapper),
  });
  ...
  const verifMapper = (v, idx) => ({
    ...
    payload: {
      ...
      explain: v.explain,
      correctAnswer:
        Array.isArray(v.options) && typeof v.a !== "undefined"
          ? v.options[v.a]
          : null,
    },
  });
  ...
  const ethicsMapper = (e, idx) => ({
    ...
    payload: {
      source: "ETHICS_SCENARIOS",
      why: e.why,
      correctAnswer: e.good ? "Es correcto" : "No es correcto",
    },
  });
  /**
  ...
  useEffect(() => {
    ...
    syncClassesRemote()
      .then(() => {
        if (mountedRef.current) setClasses(listClasses());
      })
      .catch((e) => {
    ...
    function onUpdate(e) {
      try {
        setClasses(Object.values(e.detail || listClasses()));
      } catch (_) {
        setClasses(listClasses());
      }
    }
    ...
    return () => {
      ...
      window.removeEventListener("aula-classes-updated", onUpdate);
    };
  }, []);
  ...
  useEffect(() => {
    if (activeClass) {
      initRealtime();
      try {
        subscribeToClass(activeClass, { role: "teacher" });
      } catch (e) {
      ...
      try {
        ...
        const meta = cls.meta || {};
        if (typeof meta.currentBlockIndex === "number")
          setBlockViewIndex(meta.currentBlockIndex);
      } catch (e) {
        /* ignore */
      }
    } else {
    ...
    return () => {
      /* cleanup handled globally by storage.js websocket */
    };
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        console.warn("participants poll failed", e);
      }
    };
  ...
  useRealtime(activeClass, {
    ...
    onAnswersCount: (d) => {
      setLiveAnswers((prev) => ({
        ...prev,
        [d.questionId]: { total: d.total || 0, counts: d.counts || {} },
      }));
      if (d.total >= participants.length) {
        const correct =
          questionRunning &&
          questionRunning.payload &&
          questionRunning.payload.correctAnswer
            ? questionRunning.payload.correctAnswer
            : null;
        try {
    ...
    onQuestionResults: (d) => {
      ...
      setLiveAnswers((prev) => ({
        ...prev,
        [d.questionId]: {
          total: Object.values(d.distribution || {}).reduce((a, b) => a + b, 0),
          counts: d.distribution || {},
        },
      }));
    },
    onParticipantHeartbeat: (d) => {
      setParticipants((prev) => {
        ...
        const existingName =
          copy[idx] && copy[idx].displayName ? copy[idx].displayName : null;
        const entry = {
          sessionId: d.sessionId,
          displayName:
            d.displayName ||
            existingName ||
            `Alumno-${String(d.sessionId).slice(0, 5)}`,
          score: (copy[idx] && copy[idx].score) || 0,
          lastSeen: d.lastSeen || new Date(),
          connected: d.type === "participant-heartbeat",
        };
    ...
    onClassReset: (_cls) => {
      try {
        ...
        try {
          ...
          console.warn("fetchParticipants after class-reset failed", e);
        }
        toast("Clase reiniciada (servidor)");
      } catch (e) {
        console.warn("onClassReset handler failed", e);
      }
    },
  });
            ...
            try {
              ...
              if (preferred) {
                revealQuestion(activeClass, questionRunning.id, preferred)
                  ...
                  .catch((_e) => {
                    /* si falla dejamos al profesor revelar manualmente */
                  });
              }
            } catch (e) {
              /* ignore */
  ...
  async function handleRevealAction(preferredAnswer = null) {
    ...
    const isGameEnd =
      questionRunning &&
      questionRunning.payload &&
      questionRunning.payload.type === "game-ended";
    if (!isGameEnd) setSecondsLeft(0);
    const preferred =
      preferredAnswer || questionRunning?.payload?.correctAnswer;
    const qPayload =
      questionRunning && questionRunning.payload ? questionRunning.payload : {};
    const evalMode =
      qPayload && typeof qPayload.evaluation === "string"
        ? qPayload.evaluation
        : qPayload &&
            (qPayload.source === "BAD_PROMPTS" || qPayload.source === "PROMPTS")
          ? "prompt"
          : "mcq";
    // Never ask the teacher: obtain correct answer from question payload fields
    const fromPayload =
      qPayload && typeof qPayload.correctAnswer !== "undefined"
        ? qPayload.correctAnswer
        : qPayload && typeof qPayload.correct !== "undefined"
          ? qPayload.correct
          : null;
    const correct =
      typeof preferred !== "undefined" && preferred !== null
        ? preferred
        : fromPayload;
    if (
      (evalMode === "mcq" || evalMode === "redflags") &&
      (typeof correct === "undefined" || correct === null)
    ) {
      return toast(
        "No se encontró la respuesta correcta en la pregunta. Revisa la definición en data.js",
      );
    }
    try {
      try {
        const res = await revealQuestion(
          activeClass,
          questionRunning.id,
          correct,
        );
        ...
        console.warn(
          "revealQuestion (WS) failed, attempting HTTP fallback",
          err,
        );
      ...
      try {
        const pointsToSend =
          questionRunning && typeof questionRunning.maxPoints === "number"
            ? Number(questionRunning.maxPoints)
            : questionRunning &&
                questionRunning.payload &&
                questionRunning.payload.maxPoints
              ? Number(questionRunning.payload.maxPoints)
              : 100;
        const r = await fetch(
          `/api/questions/${encodeURIComponent(questionRunning.id)}/reveal`,
          {
            method: "POST",
      ...
      toast("Error mostrando resultados: " + (e.message || e));
    }
  }
  ...
  async function handleRestartGame() {
    ...
    try {
      ...
      const res = await fetch(API_BASE + `/api/classes/${activeClass}/reset`, {
        method: "POST",
      });
      if (res.ok) {
        await res.json();
        ...
        try {
          ...
          console.warn("fetchParticipants after reset failed", e);
        }
    ...
    try {
      ...
      try {
        await fetch(API_BASE + `/api/answers?classId=${activeClass}`, {
          method: "DELETE",
        });
      } catch (e) {
  ...
  async function handleLaunch() {
    ...
    try {
      ...
      if (!meta.blocks) {
        ...
        await persistClassMeta(activeClass, meta);
        console.log("persisted initial class meta", {
          classId: activeClass,
          meta,
        });
      ...
      if (meta.finished) {
        const restart = confirm(
          "El juego ya ha finalizado. ¿Reiniciar el juego y comenzar desde el principio?",
        );
        if (!restart) {
      ...
      const currentBlockIndex =
        typeof meta.currentBlockIndex === "number" ? meta.currentBlockIndex : 0;
      const currentQuestionIndex =
        typeof meta.currentQuestionIndex === "number"
          ? meta.currentQuestionIndex
          : 0;
      console.log(
        "handleLaunch: currentBlockIndex",
        currentBlockIndex,
      ...
      const payload = {
        ...(questionToLaunch.payload || {}),
        blockId: block.id,
        blockName: block.name,
        blockIndex: currentBlockIndex,
        questionIndex: currentQuestionIndex,
      };
      const options = Array.isArray(questionToLaunch.options)
        ? questionToLaunch.options
        : [];
      const qPayload = {
        ...
        timeDecay:
          typeof questionToLaunch.timeDecay === "boolean"
            ? questionToLaunch.timeDecay
            : true,
      ...
      try {
        ...
        console.warn("subscribeToClass on launch failed", e);
      }
      ...
      try {
        const cls2 =
          classes.find((c) => (c.code || c.id) === activeClass) || {};
        ...
        console.warn("mark askedQuestions failed", e);
      }
      ...
      const isLastQuestionOfBlock =
        currentQuestionIndex === block.questions.length - 1;
      const isLastBlock =
        currentBlockIndex + 1 >= (meta.blocks ? meta.blocks.length : 0);
  ...
  async function handleNextBlock() {
    ...
    try {
      ...
      const blocks = meta.blocks || [];
      const next =
        (typeof meta.currentBlockIndex === "number"
          ? meta.currentBlockIndex
          : 0) + 1;
      ...
      meta.currentQuestionIndex =
        typeof firstUnasked === "number" && firstUnasked >= 0
          ? firstUnasked
          : 0;
      await persistClassMeta(activeClass, meta);
      setClasses(listClasses());
      ...
      console.error("handleNextBlock failed", e);
      toast("No se pudo avanzar de bloque");
    }
  }
    ...
    try {
      ...
      if (typeof firstUnasked === "number" && firstUnasked >= 0) {
        ...
        const inRange =
          curIdx !== null &&
          curIdx >= 0 &&
          curIdx < (block.questions ? block.questions.length : 0);
        const asked = meta && meta.askedQuestions ? meta.askedQuestions : {};
        const curPointsToUnasked =
          inRange && !asked[block.questions[curIdx].id];
  ...
  const activeClassData = activeClass
    ? classes.find((c) => (c.code || c.id) === activeClass)
    : null;
  const memoActiveClassData = useMemo(
    () => activeClassData,
    [activeClass, classes],
  );
