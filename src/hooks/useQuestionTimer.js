...
export default function useQuestionTimer(question) {
  ...
  const start = useCallback(() => {
    ...
    const announcedDuration =
      question && typeof question.duration === "number"
        ? Number(question.duration)
        : question &&
            question.payload &&
            typeof question.payload.duration === "number"
          ? Number(question.payload.duration)
          : 30;
  ...
  const timeTaken = (() => {
    ...
    const total =
      (question &&
        (question.duration ||
          (question.payload && question.payload.duration))) ||
      30;
