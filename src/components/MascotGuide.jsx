    ...
    if (duration && duration > 0) {
      setTimeout(() => {
        ...
        listeners.forEach((fn) =>
          fn({ text: "", mood: "neutral", duration: 0 }),
        );
      }, duration + 1000);
    }
  } catch (e) {
...
export default function MascotGuide() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (const m of pendingMessages.slice()) {
        ...
        if (now - (m.ts || 0) < 10000)
          fn({ text: m.text, mood: m.mood, duration: m.duration });
      }
    } catch (e) {}
    return () => {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("aula-chatgpt-settings", onAppSettings);
  }, []);
  ...
  return (
    <div aria-hidden={!settings.mascotVisible}>
      <div className="fixed right-6 bottom-6 z-50 flex items-end gap-3 pointer-events-none">
        {/* Speech bubble */}
        <div
          className={`max-w-xs pointer-events-auto transition-opacity ${prefersReduced ? "" : "transition-transform duration-300"} ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        >
        ...
        <div className="pointer-events-auto">
          <Mascot
            pose={pose}
            animate={animatePose && !prefersReduced}
