...
export function Timeline({
  ...
  const blockIndex =
    typeof blockViewIndex === "number"
      ? blockViewIndex
      : typeof currentMeta.currentBlockIndex === "number"
        ? currentMeta.currentBlockIndex
        : 0;

  const currentBlock =
    blocks && blocks[blockIndex] ? blocks[blockIndex] : blocks[0];
  const currentQuestions =
    currentBlock && Array.isArray(currentBlock.questions)
  ...
  if (
    currentMeta &&
    typeof currentMeta.currentQuestionIndex === "number" &&
    typeof currentMeta.currentBlockIndex === "number"
  ) {
  ...
  if (
    ...
    Number(questionRunning.payload.blockIndex) === blockIndex &&
    typeof questionRunning.payload.questionIndex === "number"
  ) {
            ...
            <div className="flex items-center justify-between gap-4 px-2 w-full">
              {currentQuestions.map((q, i) => {
                ...
                const isNext =
                  ...
                  Number(meta.currentBlockIndex) === blockIndex &&
                  typeof meta.currentQuestionIndex === "number" &&
                  Number(meta.currentQuestionIndex) === i;
                const baseClasses =
                  "w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium shadow-sm transition-transform";
