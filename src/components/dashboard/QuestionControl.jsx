...
export function QuestionControl({
  ...
  return (
    <div>
      ...
      {questionRunning &&
        questionRunning.options &&
        questionRunning.options.length > 0 && (
          <div className="grid gap-3 mb-4">
            {questionRunning.options.map((opt, i) => {
              const isCorrect =
                selectedCorrect !== null &&
                String(selectedCorrect) === String(opt);
              return (
                <div
                  key={i}
                  className={clsx(
                      ...
                      <div className="flex-1 bg-white/10 h-2 rounded overflow-hidden">
                        <div
                          style={{
                            width: `${Math.min(
                              100,
                              ((lastQuestionResults.distribution[String(opt)] ||
                                0) /
                                Math.max(
                                  1,
                                  Object.values(
                                    lastQuestionResults.distribution || {},
                                  ).reduce((a, b) => a + b, 0),
                                )) *
                                100,
                            )}%`,
      ...
      {questionRunning && (
        <div className="text-sm opacity-70 mb-4">
          ...
          {(liveAnswers[questionRunning.id] &&
            liveAnswers[questionRunning.id].total) ||
            0}
        </div>
      )}
      ...
      <div className="flex gap-3 items-center">
        ...
        <Button onClick={() => onReveal()} variant="ghost">
          Revelar
        </Button>
        <Button onClick={onShowScores} variant="ghost">
          Mostrar puntuación
        </Button>
      </div>
