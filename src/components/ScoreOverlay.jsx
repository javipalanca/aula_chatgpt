      ...
      <div className="relative z-10 bg-white rounded-xl p-6 max-w-2xl w-full text-black">
        ...
        <div className="mb-4 w-full overflow-x-auto">
          ...
          >
            {sorted.slice(0, 3).map((p, i) => (
              <div
                key={p.sessionId || i}
                className="text-center p-3 rounded-lg shadow-lg inline-block text-black"
                style={{
                  background:
                    i === 0
        ...
        <div style={{ height: 220 }}>
          <Bar
            ...
            data={{
              labels: topLabels,
              datasets: [
                { label: "Puntos", backgroundColor: colors, data: topData },
              ],
            }}
          />
        </div>
        <div className="mt-4 space-y-2 max-h-64 overflow-auto">
          {sorted.map((p) => (
            <div
              key={p.sessionId}
              className="p-2 rounded-lg border border-slate-200 flex items-center justify-between"
            >
              <div>
