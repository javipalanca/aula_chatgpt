...
export default function AulaChatGPT() {
  ...
  useEffect(() => {
    mascotSpeak({
      text: `Has cambiado a ${tab}`,
      mood: "neutral",
      duration: 2000,
    });
    try {
      window.dispatchEvent(new CustomEvent("mascot-bounce"));
    } catch {}
  }, [tab]);
  ...
  function addPoints(n, reason) {
    ...
    mascotSpeak({ text: reason || `Has ganado ${n} puntos`, mood: "happy" });
  }
  function resetStreak() {
    setStreak(0);
    mascotSpeak({ text: "Racha finalizada. Sigue intentándolo.", mood: "sad" });
  }
  function unlockBadge(name) {
    setBadges((b) => (b.includes(name) ? b : [...b, name]));
    mascotSpeak({ text: `¡Has conseguido el logro ${name}!`, mood: "cheer" });
  }

  function postScoreToClass(delta) {
    try {
      if (classCode) postParticipantUpdate(classCode, { scoreDelta: delta });
    } catch (e) {
      console.warn("postScoreToClass failed", e);
    }
  }
          ...
          <nav className="ml-auto flex gap-2 items-center">
            ...
            <button
              ...
              onClick={() =>
                setAppSettings((s) => ({
                  ...s,
                  mascotVisible: !s.mascotVisible,
                }))
              }
              className="ml-2 p-2 rounded-lg hover:bg-slate-100"
            >
              {appSettings.mascotVisible ? (
                <Eye size={16} />
              ) : (
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 flex flex-col gap-6">
          ...
          {tab === "etica" && (
            <EthicsGame
              onScore={(n) => {
                n > 0 ? addPoints(n, `+${n} puntos en Ética`) : resetStreak();
                if (n >= 8) unlockBadge("Guardián/a Ético/a");
                if (n > 0) postScoreToClass(n);
              }}
            />
          )}
          {tab === "verificacion" && (
          ...
          {tab === "diagnostico" && (
            <Diagnosis
              onScore={(ok) => {
                ...
                if (ok) {
                  unlockBadge("Cazador/a de Fakes");
                  postScoreToClass(3);
                }
              }}
            />
            ...
            <div className="mt-3 flex gap-2">
              <Button
                onClick={() => {
                  if (!TEACHER_PWD) {
                    toast(
                      "Modo docente no configurado. Añade VITE_TEACHER_PASSWORD en .env",
                    );
                    return;
                  }
              ...
              <Button onClick={() => setShowJoin(true)} variant="ghost">
                Unirse a clase
              </Button>
              {classCode && (
                <div className="ml-auto text-sm">
                  Unido a: <b>{classCode}</b>
                </div>
              )}
            </div>
          </FancyCard>
          ...
          <FancyCard>
            ...
            <div className="text-sm text-slate-700 space-y-2">
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={appSettings.mascotVisible}
                  onChange={(e) =>
                    setAppSettings((s) => ({
                      ...s,
                      mascotVisible: e.target.checked,
                    }))
                  }
                />{" "}
                Mostrar mascota
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={appSettings.mascotMuted}
                  onChange={(e) =>
                    setAppSettings((s) => ({
                      ...s,
                      mascotMuted: e.target.checked,
                    }))
                  }
                />{" "}
                Silenciar mensajes
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center p-6">
          ...
          <div className="relative w-full max-w-md bg-white rounded-xl shadow-lg p-6 z-10">
            ...
            <p className="text-sm text-slate-700 mb-4">
              Introduce la contraseña de docente para abrir el panel.
            </p>
            <input
              autoFocus
              type="password"
              ...
              placeholder="Contraseña de docente"
            />
