...
const API_BASE = (
  _RAW_API && !/^https?:\/\//i.test(_RAW_API) ? `http://${_RAW_API}` : _RAW_API
).replace(/\/$/, "");
/**
  ...
  try {
    ...
    (async () => {
      try {
        ...
        console.warn("saveProgress remote failed", e);
      }
    })();
  } catch (e) {
  ...
  try {
    ...
    if (!cached && !IN_FLIGHT.progress[id]) {
      ...
      IN_FLIGHT.progress[id] = (async () => {
        try {
          const r = await fetch(
            `${API_BASE}/api/progress/${encodeURIComponent(id)}`,
          );
          if (r.ok) {
            ...
            if (data && data.data) {
              ...
              try {
                window.dispatchEvent(
                  new CustomEvent("aula-progress", { detail: data.data }),
                );
              } catch (e) {
                console.warn("dispatch aula-progress failed", e);
              }
            }
          }
        } catch (e) {
          console.warn("loadProgress remote fetch failed", e);
        } finally {
          delete IN_FLIGHT.progress[id];
        }
      })();
    }
    return cached || null;
...
export function saveSettings(s) {
  try {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("aula-chatgpt-settings", { detail: s }),
      );
    } catch (e) {
      console.warn("dispatch settings event failed", e);
    }
    (async () => {
      try {
        await fetch(`${API_BASE}/api/settings/${encodeURIComponent(id)}`, {
          method: "PUT",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ data: s }),
        });
      } catch (e) {
        console.warn("saveSettings remote failed", e);
      }
    })();
  } catch (e) {
  ...
  try {
    ...
    if (!cached && !IN_FLIGHT.settings[id]) {
      IN_FLIGHT.settings[id] = (async () => {
        try {
          const r = await fetch(
            `${API_BASE}/api/settings/${encodeURIComponent(id)}`,
          );
          if (r.ok) {
            const data = await r.json();
            if (data && data.data) {
              ...
              try {
                window.dispatchEvent(
                  new CustomEvent("aula-chatgpt-settings", {
                    detail: data.data,
                  }),
                );
              } catch (e) {
          ...
          console.warn("loadSettings remote fetch failed", e);
        } finally {
          delete IN_FLIGHT.settings[id];
        }
      })();
    }
    ...
    return { mascotVisible: true, mascotMuted: false };
  }
}
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(
          `Remote createClass failed: ${r.status} ${r.statusText} ${body || ""}`,
        );
      }
...
export async function syncClassesRemote() {
  ...
  _classesSyncPromise = (async () => {
    try {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent("aula-classes-updated", { detail: out }),
        );
      } catch (e) {
        console.warn("dispatch aula-classes-updated failed", e);
      }
...
export async function deleteClass(code) {
  try {
    const r = await fetch(
      `${API_BASE}/api/classes/${encodeURIComponent(code)}`,
      { method: "DELETE" },
    );
...
export async function setClassActive(code, active) {
  try {
    ...
    if (!r.ok) {
      const body = await r.text().catch(() => null);
      throw new Error(
        `Failed to update class: ${r.status} ${r.statusText} ${body || ""}`,
      );
    }
    await syncClassesRemote();
    return true;
...
export async function setClassMeta(code, meta) {
  try {
    ...
    if (!r.ok) {
      const body = await r.text().catch(() => null);
      throw new Error(
        `Failed to update class meta: ${r.status} ${r.statusText} ${body || ""}`,
      );
    }
    await syncClassesRemote();
    return true;
...
export async function persistClassMeta(code, meta) {
  try {
    ...
    if (!r.ok) {
      const body = await r.text().catch(() => null);
      throw new Error(
        `Failed to persist class meta: ${r.status} ${r.statusText} ${body || ""}`,
      );
    }
    ...
    MEM.classes[code] = {
      ...
      code,
      meta,
      participants: MEM.classes[code] ? MEM.classes[code].participants : {},
      challenges: MEM.classes[code] ? MEM.classes[code].challenges : [],
      name: MEM.classes[code] ? MEM.classes[code].name : undefined,
      teacherName: MEM.classes[code]
        ? MEM.classes[code].teacherName
        : undefined,
      active: MEM.classes[code] ? MEM.classes[code].active : true,
    };
    try {
      window.dispatchEvent(
        new CustomEvent("aula-classes-updated", { detail: MEM.classes }),
      );
    } catch (e) {
      console.warn("dispatch aula-classes-updated failed", e);
    }
    ...
    try {
      ...
      console.warn("syncClassesRemote failed after persistClassMeta", e);
    }
...
export async function joinClass(code, displayName, password = null) {
  ...
  if (doc.passwordHash) {
    ...
    if (!provided || provided !== doc.passwordHash)
  ...
  const payload = {
    ...
    classId: code,
    sessionId: sid,
    displayName: displayName || `Alumno-${sid.slice(0, 5)}`,
    score: 0,
    progress: {},
    lastSeen: Date.now(),
    connected: true,
  ...
  try {
    ...
    console.warn("syncClassesRemote failed after joinClass", e);
  }
  return payload;
}
...
export async function listClassParticipants(code) {
  const r = await fetch(
    `${API_BASE}/api/participants?classId=${encodeURIComponent(code)}`,
  );
...
export async function listAnsweredQuestionIds(classId) {
  ...
  try {
    const r = await fetch(
      `${API_BASE}/api/answers?classId=${encodeURIComponent(classId)}`,
    );
    if (!r.ok) throw new Error("Failed to list answers");
    const docs = await r.json();
    const ids = new Set(
      (docs || []).filter((d) => d && d.questionId).map((d) => d.questionId),
    );
    return ids;
  } catch (e) {
      ...
      try {
        ...
        if (isWsOpen()) {
          try {
            ...
            console.warn("heartbeat ws send failed", e);
          }
        } else {
        ...
        console.warn("heartbeat sendPing failed", e);
      }
    };
...
export async function leaveClass(code) {
  try {
    ...
    await fetch(`${API_BASE}/api/participants`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(payload),
    });
  } catch (e) {
    console.warn("leaveClass failed", e);
  } finally {
    try {
      stopHeartbeat();
    } catch (e) {
      console.warn("stopHeartbeat failed", e);
    }
  }
}
...
export async function createChallenge(
  ...
  const r = await fetch(`${API_BASE}/api/challenges`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ id: cid, classId: code, ...ch }),
  });
  if (!r.ok) throw new Error("createChallenge failed");
  try {
    await syncClassesRemote();
  } catch (e) {
    console.warn("syncClassesRemote failed after createChallenge", e);
  }
  try {
    window.dispatchEvent(
      new CustomEvent("aula-challenge", { detail: { code, challenge: ch } }),
    );
  } catch (e) {
    console.warn("dispatch aula-challenge failed", e);
  }
  return ch;
}
/**
  ...
  (async () => {
    try {
      ...
      try {
        ...
        console.warn("syncClassesRemote failed in resetClass", e);
      }
    } catch (e) {
      console.warn("resetClass failed", e);
    }
  })();
}
...
export function initRealtime(baseUrl) {
  ...
  function connect() {
    ...
    if (!_ws && !triedFallback && !wsUrl.includes("localhost")) {
      ...
      try {
        console.warn(
          "initRealtime: attempting fallback websocket URL",
          fallback,
        );
    ...
    _ws.addEventListener("open", () => {
      attempts = 0;
      try {
        console.log("initRealtime: ws open", { url: wsUrl });
      } catch (e) {
        console.warn("initRealtime open log failed", e);
      }
    });
    _ws.addEventListener("message", (m) => {
      try {
        console.log("initRealtime: ws raw recv", {
          raw: String(m.data).slice(0, 200),
        });
      } catch (e) {
        console.warn("initRealtime raw recv log failed", e);
      }
      try {
        const data = JSON.parse(m.data);
        try {
          console.log("initRealtime: ws parsed recv (full)", data);
        } catch (e) {
          console.warn("initRealtime parsed recv log failed", e);
        }
        ...
        let safeDetail = data;
        try {
          safeDetail = JSON.parse(JSON.stringify(data));
        } catch (e) {
          /* if stringify fails, fallback to original */
        }
        window.dispatchEvent(
          new CustomEvent("aula-realtime", { detail: safeDetail }),
        );
      } catch (e) {
        console.warn("initRealtime: failed to parse message", e);
      }
    });
    _ws.addEventListener("close", (ev) => {
      try {
        console.log("initRealtime: ws close", ev);
      } catch (e) {
        console.warn("initRealtime close log failed", e);
      }
      _ws = null;
    ...
    _ws.addEventListener("error", (ev) => {
      try {
        console.log("initRealtime: ws error", ev);
      } catch (e) {
        console.warn("initRealtime error log failed", e);
      }
      console.warn("initRealtime websocket error", ev);
    });
  }
  try {
    connect();
  } catch (e) {
    console.warn("initRealtime initial connect failed", e);
  }
  return _ws;
}
...
export function subscribeToClass(
  ...
  try {
    ...
    try {
      console.log("subscribeToClass: preparing subscribe payload", payloadObj);
    } catch (e) {
      console.warn("subscribeToClass log failed", e);
    }
    ...
    const sendIfOpen = () => {
      ...
      try {
        if (isWsOpen()) {
          try {
            _ws.send(payload);
            console.log(
              "subscribeToClass: sent subscribe on retry",
              payloadObj,
            );
          } catch (e) {
      ...
      if (attempts >= maxAttempts) {
        clearInterval(interval);
        try {
          _ws && _ws.removeEventListener("open", onOpen);
        } catch (e) {
          console.warn("removeEventListener failed at maxAttempts", e);
        }
      }
    };
    const onOpen = () => {
    ...
    try {
      _ws && _ws.addEventListener("open", onOpen);
    } catch (e) {
      /* ignore */
    }
    ...
    setTimeout(() => {
      try {
        ...
        console.warn("subscribeToClass cleanup failed", e);
      }
    }, 11000);
    return true;
  } catch (e) {
    console.warn("subscribeToClass failed", e);
    return false;
  }
}
...
export async function createQuestion(
  ...
  if (!r.ok) {
    ...
    throw new Error(
      "createQuestion failed: " + (txt || r.status + " " + r.statusText),
    );
  }
  return q;
}
...
export function unsubscribeFromClass(classId) {
  try {
    ...
    try {
      ...
      console.warn("unsubscribeFromClass send failed", e);
      return false;
    }
  } catch (e) {
    console.warn("unsubscribeFromClass failed", e);
    return false;
  }
}
 ...
 * @returns {Promise<Object>} Payload enviado
 */
export async function submitEvaluatedAnswer(
  classId,
  sessionId,
  questionId,
  answer,
  ...
  try {
    if (isWsOpen()) {
      try {
        ...
        console.warn(
          "submitEvaluatedAnswer ws send failed, falling back to HTTP",
          e,
        );
      }
    }
  } catch (e) {
    console.warn("submitEvaluatedAnswer ws check failed", e);
  }
  ...
  try {
    if (isWsOpen()) {
      try {
        ...
        console.warn("revealQuestion ws send failed", e);
      }
    }
  } catch (e) {
    console.warn("revealQuestion ws check failed", e);
  }
