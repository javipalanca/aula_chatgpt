...
export default function App() {
  ...
  return (
    <div>
      {mode === 'landing' && (
        <Landing onEnterTeacher={() => setMode('teacher')} onJoinStudent={(code, name) => { setClassCode(code); setDisplayName(name); setMode('student') }} />
      )}
      {mode === 'student' && (
        <StudentView classCode={classCode} displayName={displayName} onBack={() => setMode('landing')} />
      )}
      {mode === 'teacher' && (
