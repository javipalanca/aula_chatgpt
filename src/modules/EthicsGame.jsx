...
export default function EthicsGame({ onScore }) {
  ...
  function answer(val) {
    ...
    if (next >= ETHICS_SCENARIOS.length) {
      ...
      mascotSpeak({
        text: `Has completado el juego de ética con ${right + (ok ? 1 : 0)} aciertos.`,
        mood:
          right + (ok ? 1 : 0) >= Math.ceil(ETHICS_SCENARIOS.length / 2)
            ? "happy"
            : "sad",
      });
    } else setI(next);
  ...
  if (finished)
    return (
      <FancyCard>
        ...
        <p>
          ¡Has acertado {right} de {ETHICS_SCENARIOS.length}!
        </p>
      </FancyCard>
    );
