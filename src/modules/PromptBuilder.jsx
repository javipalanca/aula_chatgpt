...
export default function PromptBuilder({ onScore }) {
  ...
  async function tryOllama() {
    if (!OLLAMA_URL)
      return toast(
        "La URL del servidor de inferencia no está configurada en .env",
      );
    setOllamaResp("");
    ...
    try {
        ...
        for (const line of lines) {
          ...
          if (trimmed.startsWith("data:")) {
            ...
            try {
              ...
              console.warn("prompt builder inner parse failed", e);
            }
          }
          ...
          try {
            ...
            console.warn("prompt builder parse failed", e);
          }
      ...
      setOllamaResp(final);
      mascotSpeak({
        text: "He generado una respuesta. Puedes revisarla.",
        mood: "happy",
      });
    } catch (err) {
      console.error(err);
      toast(
        "Error al conectar con el servidor de inferencia: " +
          (err.message || err),
      );
      setOllamaResp("Error: " + (err.message || String(err)));
      mascotSpeak({
        text: "No he podido obtener respuesta del servidor.",
        mood: "sad",
      });
    } finally {
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={tryOllama}
              variant="ghost"
              disabled={!OLLAMA_URL || ollamaLoading}
            >
              {ollamaLoading ? "Generando…" : "Generar respuesta"}
            </Button>
          </div>
          {OLLAMA_URL ? (
            <div className="mt-3">
              ...
              <pre className="whitespace-pre-wrap bg-slate-50 rounded-xl p-3 text-sm min-h-[80px]">
                {ollamaResp ||
                  (ollamaLoading
                    ? "Esperando respuesta…"
                    : 'Pulsa "Generar respuesta" para ver la respuesta')}
              </pre>
            </div>
          ) : (
            <p className="mt-2 text-xs text-rose-600">
              La URL del servidor de inferencia no está configurada. Añade
              VITE_OLLAMA_URL en tu archivo `.env` (ver README).
            </p>
          )}
        </div>
      </div>
