...
describe('LLMEvaluator', () => {
  ...
  vi.stubGlobal('fetch', async (_url, _opts) => {
      return {
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ choices: [{ message: { content: '{"score":80,"feedback":"bien"}' } }] })
      }
    })
    const ev = new LLMEvaluator({ openaiKey: 'k', openaiUrl: 'https://api.openai.com', openaiModel: 'm' })
  ...
  it('falls back to Ollama when OpenAI fails', async () => {
    vi.stubGlobal('fetch', async (_url, _opts) => {
      ...
      return { ok: true, status: 200, text: async () => JSON.stringify({ results: [{ content: '{"score":50,"feedback":"okollama"}' }] }) }
    })
    const ev = new LLMEvaluator({ openaiKey: 'k', openaiUrl: 'https://api.openai.com', openaiModel: 'm', ollamaUrl: 'http://ollama.local', ollamaModel: 'll' })
    const res = await ev.evaluate({ title: 'Q' }, 'respuesta')
  ...
  vi.stubGlobal('fetch', async (_url, _opts) => ({ ok: true, status: 200, text: async () => 'no json here' }))
    const ev = new LLMEvaluator({ openaiKey: 'k', openaiUrl: 'https://api.openai.com', openaiModel: 'm', ollamaUrl: 'http://ollama.local', ollamaModel: 'll' })
    const res = await ev.evaluate({ title: 'Q' }, 'respuesta')
