...
describe('ParticipantService', () => {
  ...
  beforeEach(() => {
    ...
    svc = new ParticipantService({ participantsRepo, broadcast, participantLastPersist, participantLastBroadcast, options: { minPersistMs: 5000, minBroadcastMs: 2000 } })
  })

  it('saveParticipant skips writes when recent and not a score op', async () => {
    const payload = { id: 'c1:s1', classId: 'c1', sessionId: 's1', displayName: 'A' }
  ...
  it('saveParticipant calls incScore when scoreDelta provided', async () => {
    const payload = { id: 'c2:s2', classId: 'c2', sessionId: 's2', scoreDelta: 7 }
    const res = await svc.saveParticipant(payload)
  ...
  it('handleSubscribe upserts participant and broadcasts participants-updated', async () => {
    ...
    participantsRepo.listConnected.mockResolvedValue([{ sessionId: 'sX', displayName: 'X' }])
    const result = await svc.handleSubscribe({ classId: 'cls1', sessionId: 'sX', role: 'student', displayName: 'Tester' })
    expect(result.ok).toBe(true)
    expect(participantsRepo.upsert).toHaveBeenCalled()
    expect(broadcast).toHaveBeenCalled()
    const call = broadcast.mock.calls.find(c => c[0] && c[0].type === 'participants-updated')
    expect(call).toBeTruthy()
  })

  it('handleDisconnect marks disconnected and broadcasts', async () => {
    participantsRepo.listConnected.mockResolvedValue([{ sessionId: 'sY', displayName: 'Y' }])
    await svc.handleDisconnect('cls2', 'sY')
    expect(participantsRepo.markDisconnected).toHaveBeenCalledWith('cls2', 'sY')
  ...
  it('handlePing throttles heartbeat broadcast when recently broadcasted', async () => {
    ...
    participantsRepo.findOneByClassSession.mockResolvedValue({ connected: true, displayName: 'Z' })
  ...
  it('saveParticipant rejects when repo.upsert fails', async () => {
    ...
    const payload = { id: 'err:1', classId: 'err', sessionId: '1', displayName: 'Err' }
  ...
  it('handleSubscribe propagates repo error', async () => {
    ...
    await expect(svc.handleSubscribe({ classId: 'cE', sessionId: 'sE', role: 'student', displayName: 'E' })).rejects.toThrow('Upsert fail')
  })

  it('handlePing triggers upsert when lastPersist expired', async () => {
    participantsRepo.findOneByClassSession.mockResolvedValue({ connected: true, displayName: 'Old' })
