...
function makeFakeCollection() {
  const docs = new Map()
  return {
    async replaceOne(filter, doc, _opts) { const id = doc.id || (filter && filter.id); if (!id) throw new Error('id required'); docs.set(id, { ...doc }); return { acknowledged: true } },
    find(q) { const out = Array.from(docs.values()).filter(d => { for (const k of Object.keys(q||{})) if (d[k] !== q[k]) return false; return true }); return { toArray: async () => out } },
    async findOne(q) { for (const v of docs.values()) { let ok = true; for (const k of Object.keys(q||{})) if (v[k] !== q[k]) { ok = false; break } if (ok) return v } return null },
...
describe('ProgressRepo', () => {
  let fakeCol
  beforeEach(() => { fakeCol = makeFakeCollection(); ProgressRepo.prototype._col = () => fakeCol })
