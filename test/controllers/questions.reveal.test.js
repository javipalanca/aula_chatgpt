...
describe("questions controller - reveal endpoint", () => {
  ...
  beforeEach(() => {
    app = createApp();
    mockQuestionService = {
      revealQuestion: vi.fn().mockResolvedValue({ ok: true, distribution: {} }),
    };
    const qc = questionsControllerFactory({
  ...
  it("delegates to questionService.revealQuestion and returns its result", async () => {
    ...
    expect(mockQuestionService.revealQuestion).toHaveBeenCalledWith(
