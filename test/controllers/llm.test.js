...
describe('llm controller', () => {
  ...
  beforeEach(() => {
    ...
    const llmController = llmControllerFactory({ evaluator, ollamaConfig: { url: 'http://ollama', model: 'm' }, fetchImpl: vi.fn() })
    app = createApp()
  ...
  it('POST /api/llm/evaluate calls evaluator', async () => {
    const res = await request(app).post('/api/llm/evaluate').send({ question: { id: 'Q' }, answer: 'a' })
