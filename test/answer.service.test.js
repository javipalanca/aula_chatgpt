...
describe("AnswerService", () => {
  ...
  it("submitAnswer upserts and broadcasts counts", async () => {
    ...
    answersRepo.findByClassQuestion.mockResolvedValue([
      { answer: "a" },
      { answer: "b" },
    ]);
  ...
  it("submitAnswer handles client-provided evaluation and awards points", async () => {
    const activeQuestion = {
      question: { payload: { evaluation: "open", points: 100, duration: 30 } },
    ...
    const hasEvalUpsert = answersRepo.upsert.mock.calls.some(
      (call) =>
        call[0] && call[0].evaluation && call[0].evaluation.source === "client",
    );
  ...
  it("submitAnswer uses evaluator when no client evaluation and awards points", async () => {
    const activeQuestion = {
      question: { payload: { evaluation: "prompt", points: 50, duration: 30 } },
    ...
    expect(participantsRepo.incScore).toHaveBeenCalled();
    const hasServerEval = answersRepo.upsert.mock.calls.some(
      (call) =>
        call[0] && call[0].evaluation && call[0].evaluation.source === "server",
    );
